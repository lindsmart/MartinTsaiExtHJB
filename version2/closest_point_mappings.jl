...
function init_closest_point_mapping!(point_cloud_kdtree,
                                    ...
                                    ϵ::Float64,
                                    h::Float64,
                                    x_range::Array{Float64,1},
                                    y_range::Array{Float64,1},
...
function CPonSinterp(kdtree, gridpoint::Array{Float64,1}, surfdata::Array{Float64,2},testpoints)
    ...
    #else
        #idxsCP=k_closests_points(kdtree, gridpoint,surfdata,6)
...
function compute_cp_mapping(n1::Int64,
    ...
    x_range=range(0.0, stop=1.0, length=M)
    y_range=range(0.0, stop=1.0, length=M)
    z_range=range(0.0, stop=1.0, length=M)

    w = Int64(ceil(max_eps/h))
    h=x_range[2]-x_range[1]

    #updating the max_eps neighborhood of each point in point_cloud
    for I=1:n_points
        ...
        #need to optimize for grid ordering
        for k=k0:k1, j=j0:j1, i=i0:i1
            #check if point is within max_eps distance and the closest point has
            # not already been computed
            if U[i,j,k]>=1000
                gridpoint=[(i-1)*dx,(j-1)*dy,(k-1)*dz]
                idx, dist = knn(point_cloud_kdtree, gridpoint, 1, true)
